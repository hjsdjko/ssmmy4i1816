  ...
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          yaoshizhanghao: [
            { required: true, message: '药师账号不能为空', trigger: 'blur' },
          ],
          mima: [
            { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          yaoshixingming: [
            { required: true, message: '药师姓名不能为空', trigger: 'blur' },
          ],
          xingbie: [
          ],
          nianling: [
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`yaoshi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('yaoshi/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`yaoshi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
