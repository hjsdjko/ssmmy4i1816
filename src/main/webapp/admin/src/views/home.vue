...
<div class="content" :style='{"minHeight":"100vh","padding":"0px 30px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url() no-repeat center top / cover,#e9ecf3","display":"block","fontSize":"14px","height":"auto"}'>
	...
	<div :style='{"border":"0px solid #bababa","padding":"0","margin":"10px 0 20px","alignItems":"flex-start","color":"#fff","display":"flex","justifyContent":"center","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"none","width":"100%","fontSize":"14px","order":"0"}'>
		<div :style='{"padding":"20px","margin":"0 2% 12px 0","borderColor":"#e7e8f8","alignItems":"center","textAlign":"center","display":"flex","float":"left","justifyContent":"flex-start","overflow":"hidden","borderRadius":"5px","background":"#3598dc","borderWidth":"0px","flex":"1","width":"18%","position":"relative","borderStyle":"solid"}' v-if="isAuth('yaowuxinxi','首页总数')">
			...
			<div :style='{"margin":"0 0 0 20px","alignItems":"flex-end","textAlign":"left","flexDirection":"column","background":"none","display":"flex","width":"calc(100% - 70px)","justifyContent":"center"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"34px","color":"inherit","fontWeight":"500","height":"24px"}'>{{yaowuxinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":"inherit","height":"24px"}'>药物信息总数</div>
			</div>
		</div>
		<div :style='{"padding":"20px","margin":"0 2% 12px 0","borderColor":"#e7f8f8","alignItems":"center","textAlign":"center","display":"flex","float":"left","justifyContent":"flex-start","overflow":"hidden","borderRadius":"5px","background":"#e7505a","borderWidth":"0px","flex":"1","width":"18%","position":"relative","borderStyle":"solid"}' v-if="isAuth('chufangxinxi','首页总数')">
			<div :style='{"alignItems":"center","borderRadius":"100%","background":"none","display":"flex","width":"50px","justifyContent":"center","height":"50px"}'>
			...
			<div :style='{"margin":"0 0 0 20px","alignItems":"flex-end","textAlign":"left","flexDirection":"column","display":"flex","width":"calc(100% - 70px)","justifyContent":"center"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"34px","color":"inherit","fontWeight":"500","height":"24px"}'>{{chufangxinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":"inherit","height":"24px"}'>处方信息总数</div>
			</div>
		</div>
		<div :style='{"padding":"20px","margin":"0 2% 12px 0","borderColor":"#e7f8e8","alignItems":"center","textAlign":"center","display":"flex","float":"left","justifyContent":"flex-start","overflow":"hidden","borderRadius":"5px","background":"#32c5d2","borderWidth":"0px","flex":"1.2","width":"18%","position":"relative","borderStyle":"solid"}' v-if="isAuth('peiyaoxinxi','首页总数')">
			<div :style='{"alignItems":"center","borderRadius":"100%","background":"none","display":"flex","width":"50px","justifyContent":"center","height":"50px"}'>
			...
			<div :style='{"margin":"0 0 0 20px","alignItems":"flex-end","textAlign":"left","flexDirection":"column","display":"flex","width":"calc(100% - 70px)","justifyContent":"center"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"34px","color":"inherit","fontWeight":"500","height":"24px"}'>{{peiyaoxinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":"inherit","height":"24px"}'>配药信息总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type3" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto","order":"10"}'>
		<div id="yaowuxinxiChart1" class="echarts1" v-if="isAuth('yaowuxinxi','首页统计')"></div>
		<div id="chufangxinxiChart1" class="echarts2" v-if="isAuth('chufangxinxi','首页统计')"></div>
		<div id="peiyaoxinxiChart1" class="echarts3" v-if="isAuth('peiyaoxinxi','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getyaowuxinxiCount();
		this.yaowuxinxiChat1();
		this.getchufangxinxiCount();
		this.chufangxinxiChat1();
		this.getpeiyaoxinxiCount();
		this.peiyaoxinxiChat1();
	},
	methods:{
		// 词云
		...
		getyaowuxinxiCount() {
			this.$http({
				url: `yaowuxinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.yaowuxinxiCount = data.data
				}
			})
		},
		yaowuxinxiChat1() {
        ...
        var yaowuxinxiChart1 = echarts.init(document.getElementById("yaowuxinxiChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                yaowuxinxiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yaowuxinxiChart1.resize();
                };
            }
        });
      })
		...
		getchufangxinxiCount() {
			this.$http({
				url: `chufangxinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.chufangxinxiCount = data.data
				}
			})
		},
		chufangxinxiChat1() {
        ...
        var chufangxinxiChart1 = echarts.init(document.getElementById("chufangxinxiChart1"),'macarons');
        this.$http({
            url: "chufangxinxi/group/yishengxingming",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                chufangxinxiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    chufangxinxiChart1.resize();
                };
            }
        });
      })
		...
		getpeiyaoxinxiCount() {
			this.$http({
				url: `peiyaoxinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.peiyaoxinxiCount = data.data
				}
			})
		},
		peiyaoxinxiChat1() {
        ...
        var peiyaoxinxiChart1 = echarts.init(document.getElementById("peiyaoxinxiChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                peiyaoxinxiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    peiyaoxinxiChart1.resize();
                };
            }
        });
      })
